...
module.exports = async (fastify, opts) => {
    ...
    fastify.get('/check/versions', async (request, reply) => {
        ...
        return reply.code(200).send({
            current: {
                api: `v${await sendGithubRequest({ repo: 'CordXApp/OldWebsite', branch: 'master', path: 'package.json'})}`, 
                client: `v${await sendGithubRequest({ repo: 'CordXApp/Client', branch: 'master', path: 'package.json'})}`,
                website: `v${await sendGithubRequest({ repo: 'CordXApp/OldWebsite', branch: 'master', path: 'package.json'})}`
            },
            newest: {
